...
export function EditTransactionForm({ user, isLoading, hasToken }: Props) {
  ...
  useEffect(() => {
    if (transaction) {
      ...
      setValue("paidByUserId", transaction.paidByUserId);
      setValue("groupId", transaction.group?.id);
      if (transaction.splits && transaction.splits.length > 0) {
        setValue("splits", transaction.splits);
      }
    }
